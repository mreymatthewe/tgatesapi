...
const TokenMetadataInputSchema = CommonContractSchema.extend({
  ...
  initialSupply: QueryParams.AmountSchema
});
/**
 * @public
 */
...
class Deployer {
  ...
  async createNftDrop(metadata) {
    ...
    const regTx = js.TransactionBuilder.make().add(...registryInstructions).setFeePayer(this.metaplex.identity()).setTransactionOptions({
      blockhash: block.blockhash,
      feePayer: this.metaplex.identity().publicKey,
      lastValidBlockHeight: block.lastValidBlockHeight
    });
...
class NFTHelper {
  ...
  async ownerOf(nftAddress) {
    try {
      ...
      const connection = new web3_js.Connection(getPublicRpc(this.metaplex), "confirmed");
      const largestAccounts = await connection.getTokenLargestAccounts(new web3_js.PublicKey(nftAddress));
  ...
  async getAll(collectionAddress, queryParams) {
    ...
    const count = (queryParams === null || queryParams === void 0 ? void 0 : queryParams.count) || QueryParams.DEFAULT_QUERY_ALL_COUNT; // TODO cache signatures <> transactions mapping in memory so pagination doesn't re-request this everytime
    ...
    const metadataInfos = await js.GmaBuilder.make(this.metaplex, fixedMetadataAddresses).getBetween(start, start === 0 ? count + 1 : start + count); // parse each account into a metadata account
...
class NFTCollection {
  ...
  async mintAdditionalSupplyTo(to, nftAddress, amount) {
    ...
    invariant__default["default"](isNaN(amount) === false, "amount must be possible to convert to a number"); // Better to use metaplex functions directly then our supplyOf function for types/consistency
...
class ClaimConditions {
  ...
  async set(metadata) {
    ...
    const goLiveDate = parsed.startTime ? js.toDateTime(parsed.startTime) : undefined; // max claimable
