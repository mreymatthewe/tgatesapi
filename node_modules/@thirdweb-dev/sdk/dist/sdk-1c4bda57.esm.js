import { _ as _defineProperty, F as FileOrBufferOrStringSchema, P as PercentSchema, A as AmountSchema, a as BasisPointsSchema, Q as QuantitySchema, g as getRpcUrl, D as DEFAULT_QUERY_ALL_COUNT, b as CommonNFTInput } from './QueryParams-237af208.esm.js';
import { z } from 'zod';
import { PublicKey, Keypair, clusterApiUrl, Connection } from '@solana/web3.js';
import { guestIdentity, amount, SOL, isKeypairSigner, keypairIdentity, isIdentitySigner, walletAdapterIdentity, token, findMetadataPda, toBigNumber, sol, TransactionBuilder, getSignerHistogram, GmaBuilder, toMetadata, toMetadataAccount, findMasterEditionV2Pda, toNftOriginalEdition, toOriginalEditionAccount, toDateTime, resolveClusterFromConnection, Pda, TokenProgram, Metaplex } from '@metaplex-foundation/js';
...
class WalletAuthenticator {
  ...
  async authenticate(domain, token) {
    ...
    if ((connectedAddress === null || connectedAddress === void 0 ? void 0 : connectedAddress.toLowerCase()) !== payload.iss.toLowerCase()) {
...
class Deployer {
  ...
  async createNftDrop(metadata) {
    ...
    const signedTx = await this.metaplex.identity().signAllTransactions([dropTransaction, regTx.toTransaction()]); // send the signed transactions *sequentially* the drop creation needs to succeed first before adding to registry
...
function getNework(metaplex) {
  const url = new URL(metaplex.connection.rpcEndpoint); // try this first to avoid hitting `custom` network for alchemy urls
...
class NFTHelper {
  ...
  async ownerOf(nftAddress) {
    try {
      var _largestAccountInfo$v;

      // TODO switch back to normal connection when alchemy supports getTokenLargestAccounts
      const connection = new Connection(getPublicRpc(this.metaplex), "confirmed");
      const largestAccounts = await connection.getTokenLargestAccounts(new PublicKey(nftAddress));
      const largestAccountInfo = await connection.getParsedAccountInfo(largestAccounts.value[0].address);
      const parsedData = largestAccountInfo === null || largestAccountInfo === void 0 ? void 0 : (_largestAccountInfo$v = largestAccountInfo.value) === null || _largestAccountInfo$v === void 0 ? void 0 : _largestAccountInfo$v.data;
  ...
  async getAll(collectionAddress, queryParams) {
    ...
    const start = (queryParams === null || queryParams === void 0 ? void 0 : queryParams.start) || 0;
    const count = (queryParams === null || queryParams === void 0 ? void 0 : queryParams.count) || DEFAULT_QUERY_ALL_COUNT; // TODO cache signatures <> transactions mapping in memory so pagination doesn't re-request this everytime
    ...
    const metadataAddresses = []; // TODO RPC's will throttle this, need to do some optimizations here
    ...
    const metadataInfos = await GmaBuilder.make(this.metaplex, fixedMetadataAddresses).getBetween(start, start === 0 ? count + 1 : start + count); // parse each account into a metadata account
  ...
  async toNFTMetadata(meta) {
    ...
    if (meta.model === "metadata") {
      fullModel = await this.metaplex.nfts().findByMint({
...
async function sendMultipartTransaction(builders, metaplex) {
  ...
  const txns = builders.map(builder => {
    const builderTx = builder.setTransactionOptions({
      blockhash: block.blockhash,
  ...
  const signedTx = await metaplex.identity().signAllTransactions(txns); // send the signed transactions

  let signatures = [];

  for (const tx of signedTx) {
    const signature = await metaplex.connection.sendRawTransaction(tx.serialize());
...
class NFTCollection {
  ...
  async mintAdditionalSupplyTo(to, nftAddress, amount) {
    ...
    amount = typeof amount === "string" ? parseInt(amount) : amount; // ensure that whatever got passed actually is a number now

    invariant(isNaN(amount) === false, "amount must be possible to convert to a number"); // Better to use metaplex functions directly then our supplyOf function for types/consistency
...
class ClaimConditions {
  ...
  async get() {
    ...
    const isWithinGoLiveDate = candyMachine.goLiveDate ? candyMachine.goLiveDate.toNumber() * 1000 >= Date.now() : true;
    ...
    const isBelowMaxClaimable = maxClaimable !== "unlimited" ? claimedSupply < maxClaimable : claimedSupply < lazyMintedSupply; // TODO add allowlist/hidden settings here
  ...
  async set(metadata) {
    ...
    const goLiveDate = parsed.startTime ? toDateTime(parsed.startTime) : undefined; // max claimable

    const endSettings = parsed.maxClaimable ? parsed.maxClaimable === "unlimited" ? null : {
      endSettingType: 1,
      number: toBigNumber(parsed.maxClaimable)
    } : undefined; // TODO add allowlist/hidden settings here
...
class NFTDrop {
  ...
  async lazyMint(metadatas, options) {
    ...
    const builders = batches.map((batch, i) => this.metaplex.candyMachines().builders().insertItems({
      candyMachine,
  ...
  async claimTo(receiverAddress, amount) {
    ...
    const builders = await Promise.all([...Array(amount).keys()].map(async () => {
      return await this.metaplex.candyMachines().builders().mint({
        candyMachine,
  ...
  async updateCreators(creators) {
    const tx = await this.metaplex.candyMachines().update({
      candyMachine: await this.getCandyMachine(),
      creators: enforceCreator(creators, this.metaplex.identity().publicKey)
    }).run();
    return {
